...
class GuiSettingsManager:
    ...
    def list_addons(self):
        ...
        addons = json.loads(
            xbmc.executeJSONRPC(
                '{"jsonrpc":"2.0", "method":"Addons.GetAddons", "params":{"properties":["version","author"]}, "id":2}'
            )
        )
    ...
    def restore(self, restoreSettings):
        ...
        for aSetting in restoreSettings:
            ...
            if aSetting["id"] in settingsDict.values():
                ...
                if (
                    aSetting["type"] != "action"
                    and settingsDict[aSetting["id"]] != aSetting["value"]
                ):
                    if utils.getSettingBool("verbose_logging"):
                        utils.log(
                            "%s different than current: %s"
                            % (aSetting["id"], str(aSetting["value"]))
                        )
