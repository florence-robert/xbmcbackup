...
class XbmcBackup:
    ...
    restore_point = None
    skip_advanced = (
        False  # if we should check for the existance of advancedsettings in the restore
    )
    ...
    def listBackups(self, reverse=True):
        ...
        for aFile in files:
            ...
            if (
                file_ext == "zip"
                and len(folderName) >= 12
                and folderName[0:12].isdigit()
            ):
                # format the name according to regional settings and display the file size
                folderName = "%s - %s" % (
                    self._dateFormat(folderName),
                    utils.diskString(
                        self.remote_vfs.fileSize(self.remote_base_path + aFile)
                    ),
                )
    ...
    def backup(self, progressOverride=False):
        ...
        if shouldContinue:
            ...
            utils.log(
                "File Selection Type: " + str(utils.getSetting("backup_selection_type"))
            )
            allFiles = []
            ...
            if utils.getSettingInt("backup_selection_type") == 0:
                ...
                for aDir in self.simple_directory_list:
                    ...
                    if utils.getSettingBool("backup_" + aDir):
                        ...
                        allFiles.append(
                            self._addBackupDir(
                                aDir,
                                selectedDirs[aDir]["root"],
                                selectedDirs[aDir]["dirs"],
                            )
                        )
            else:
                ...
                for aKey in keys:
                    ...
                    allFiles.append(
                        self._addBackupDir(aKey, aSet["root"], aSet["dirs"])
                    )
            ...
            if not writeCheck:
                ...
                shouldContinue = xbmcgui.Dialog().yesno(
                    utils.getString(30089),
            ...
            for fileGroup in allFiles:
                ...
                filesCopied = self._copyFiles(
                    fileGroup["files"], self.xbmc_vfs, self.remote_vfs
                )
            ...
            if utils.getSettingBool("compress_backups"):
                ...
                zip_name = os.path.join(
                    self.ZIP_TEMP_PATH, self.remote_vfs.root_path[:-1] + ".zip"
                )
                self.remote_vfs.cleanup()
                self.xbmc_vfs.rename(
                    os.path.join(self.ZIP_TEMP_PATH, "xbmc_backup_temp.zip"), zip_name
                )
                ...
                fileCopied = self._copyFiles(
                    fileManager.getFiles(), self.xbmc_vfs, self.remote_vfs
                )

                if not fileCopied:
                    # zip archive copy filed, inform the user
                    shouldContinue = xbmcgui.Dialog().ok(
                        utils.getString(30089),
        ...
        if shouldContinue:
            ...
            if self.restore_point.split(".")[-1] == "zip":
                ...
                if not self.xbmc_vfs.exists(restore_path):
                    ...
                    self.transferSize = self.remote_vfs.fileSize(
                        self.remote_base_path + self.restore_point
                    )
                    zipFile = []
                    zipFile.append(
                        {
                            "file": self.remote_base_path + self.restore_point,
                            "size": self.transferSize,
                        }
                    )
                ...
                self.remote_vfs = XBMCFileSystem(
                    os.path.join(self.ZIP_TEMP_PATH, self.restore_point.split(".")[0])
                )
            ...
            if (
                self.remote_vfs.exists(
                    self.remote_vfs.root_path + "config/advancedsettings.xml"
                )
                and not self.skip_advanced
            ):
                # let the user know there is an advanced settings file present
                restartXbmc = xbmcgui.Dialog().yesno(
                    utils.getString(30038),
                    "%s\n%s\n%s"
                    % (
                        utils.getString(30039),
                        utils.getString(30040),
                ...
                if restartXbmc:
                    ...
                    self._copyFiles(
                        fileManager.getFiles(), self.remote_vfs, self.xbmc_vfs
                    )
            ...
            if selectedSets is None:
                selectedSets = xbmcgui.Dialog().multiselect(
                    utils.getString(30131), restoreSets
                )
            else:
                selectedSets = [
                    restoreSets.index(n) for n in selectedSets if n in restoreSets
                ]  # if set name not found just skip it
            ...
            if selectedSets is not None:
                ...
                for index in selectedSets:
                    ...
                    if self.remote_vfs.exists(
                        ...
                        allFiles.append(
                            {
                                "source": self.remote_vfs.root_path + aDir["name"],
                                "dest": self.xbmc_vfs.root_path,
            ...
            if self.restore_point.split(".")[-1] == "zip":
                ...
                self.xbmc_vfs.rmfile(
                    os.path.join(self.ZIP_TEMP_PATH, self.restore_point)
                )
                xbmc.sleep(1000)
                self.xbmc_vfs.rmdir(
                    self.remote_vfs.clean_path(
                        os.path.join(
    ...
    def _setupVFS(self, mode=-1, progressOverride=False):
        ...
        if mode == self.Backup and self.remote_vfs.root_path != "":
            ...
            self.remote_vfs.set_root(
                ...
                + utils.getSetting("backup_suffix").strip()
                + "/"
            )
            progressBarTitle = (
                progressBarTitle
                + utils.getString(30023)
                + ": "
                + utils.getString(30016)
            )
        elif (
            mode == self.Restore
            and self.restore_point is not None
            and self.remote_vfs.root_path != ""
        ):
            if self.restore_point.split(".")[-1] != "zip":
                self.remote_vfs.set_root(
                    self.remote_vfs.root_path + self.restore_point + "/"
                )
            progressBarTitle = (
                progressBarTitle
                + utils.getString(30023)
        ...
        for aFile in fileList:
            if not self.progressBar.checkCancel():
                ...
                if aFile["file"].startswith("-"):
                    ...
                    dest.mkdir(
                        dest.root_path + aFile["file"][len(source.root_path) + 1 :]
                    )
                else:
                    self._updateProgress(
                        "%s remaining\nwriting %s"
                    ...
                    wroteFile = self._copyFile(
                        ...
                        dest,
                        aFile["file"],
                        dest.root_path + aFile["file"][len(source.root_path) :],
                    )
    ...
    def _updateProgress(self, message=None):
        self.progressBar.updateProgress(
            int(
                (
                    float(self.transferSize - self.transferLeft)
                    / float(self.transferSize)
                )
                * 100
            ),
            message,
    ...
    def _rotateBackups(self):
        ...
        if total_backups > 0:
            ...
            if len(dirs) > total_backups:
                ...
                while (
                    remove_num < (len(dirs) - total_backups)
                    and not self.progressBar.checkCancel()
                ):
                    self._updateProgress(
                        utils.getString(30054) + " " + dirs[remove_num][1]
                    )
                    utils.log("Removing backup " + dirs[remove_num][0])

                    if dirs[remove_num][0].split(".")[-1] == "zip":
                        # this is a file, remove it that way
                        self.remote_vfs.rmfile(
                            self.remote_vfs.clean_path(self.remote_base_path)
                            + dirs[remove_num][0]
                        )
                    else:
                        self.remote_vfs.rmdir(
                            self.remote_vfs.clean_path(self.remote_base_path)
                            + dirs[remove_num][0]
                            + "/"
                        )
    ...
    def _createValidationFile(self, dirList):
        valInfo = {
            "name": "XBMC Backup Validation File",
        ...
        vFile = xbmcvfs.File(
            xbmcvfs.translatePath(utils.data_dir() + "xbmcbackup.val"), "w"
        )
        ...
        success = self._copyFile(
            self.xbmc_vfs,
            self.remote_vfs,
            xbmcvfs.translatePath(utils.data_dir() + "xbmcbackup.val"),
            self.remote_vfs.root_path + "xbmcbackup.val",
        )
        ...
        if success:
            ...
            if not xbmcvfs.exists(xbmcvfs.translatePath(utils.data_dir() + ".nomedia")):
                nmFile = xbmcvfs.File(
                    xbmcvfs.translatePath(utils.data_dir() + ".nomedia"), "w"
                )
                nmFile.close()
            ...
            success = self._copyFile(
                self.xbmc_vfs,
                self.remote_vfs,
                xbmcvfs.translatePath(utils.data_dir() + ".nomedia"),
                self.remote_vfs.root_path + ".nomedia",
            )
    ...
    def _checkValidationFile(self, path):
        ...
        self._copyFile(
            ...
            self.xbmc_vfs,
            path + "xbmcbackup.val",
            xbmcvfs.translatePath(utils.data_dir() + "xbmcbackup_restore.val"),
        )

        with xbmcvfs.File(
            xbmcvfs.translatePath(utils.data_dir() + "xbmcbackup_restore.val"), "r"
        ) as vFile:
    ...
    def _createResumeBackupFile(self):
        with xbmcvfs.File(
            xbmcvfs.translatePath(utils.data_dir() + "resume.txt"), "w"
        ) as f:
    ...
    def walkTree(self, directory, recurse=True):
        ...
        if self.vfs.exists(directory + self.pathSep):
            ...
            if recurse:
                ...
                for aDir in dirs:
                    dirPath = xbmcvfs.validatePath(
                        xbmcvfs.translatePath(directory + self.pathSep + aDir)
                    )
    ...
    def addDir(self, dirMeta):
        if dirMeta["type"] == "include":
            self.root_dirs.append(
                {"path": dirMeta["path"], "recurse": dirMeta["recurse"]}
            )
        else:
