...
class BackupSetManager:
    ...
    def addSet(self, aSet):
        self.paths[aSet["name"]] = {
            "root": aSet["root"],
            "dirs": [{"type": "include", "path": aSet["root"], "recurse": True}],
        }
...
class AdvancedBackupEditor:
    ...
    def createSet(self):
        ...
        if name is not None:
            ...
            enterHome = self.dialog.yesno(
                utils.getString(30111),
                message=utils.getString(30112)
                + " - "
                + utils.getString(30114)
                + "\n"
                + utils.getString(30113)
                + " - "
                + utils.getString(30115),
                nolabel=utils.getString(30112),
                yeslabel=utils.getString(30113),
            )

            rootFolder = "special://home"
            if enterHome:
                rootFolder = self.dialog.input(
                    utils.getString(30116), defaultt=rootFolder
                )
                ...
                if not xbmcvfs.exists(xbmcvfs.translatePath(rootFolder)):
                    self.dialog.ok(
                        utils.getString(30117), utils.getString(30118), rootFolder
                    )
                    return None
            else:
                # select path to start set
                rootFolder = self.dialog.browse(
                    type=0,
                    heading=utils.getString(30119),
                    shares="files",
                    defaultt=rootFolder,
                )
    ...
    def editSet(self, name, backupSet):
        ...
        while optionSelected != -1:
            options = [
                xbmcgui.ListItem(utils.getString(30120), utils.getString(30143)),
                xbmcgui.ListItem(utils.getString(30135), utils.getString(30144)),
                xbmcgui.ListItem(rootPath, utils.getString(30121)),
            ]
            ...
            for aDir in backupSet["dirs"]:
                if aDir["type"] == "exclude":
                    options.append(
                        xbmcgui.ListItem(
                            self._cleanPath(rootPath, aDir["path"]),
                            "%s: %s" % (utils.getString(30145), utils.getString(30129)),
                        )
                    )
                    ...
                    options.append(
                        xbmcgui.ListItem(
                            ...
                            "%s: %s | %s: %s"
                            % (
                                utils.getString(30145),
                                utils.getString(30134),
                                utils.getString(30146),
                                str(aDir["recurse"]),
                            ),
                        )
                    )

            optionSelected = self.dialog.select(
                utils.getString(30122) + " " + name, options, useDetails=True
            )

            if optionSelected == 0 or optionSelected == 1:
                # add a folder, will equal root if cancel is hit
                addFolder = self.dialog.browse(
                    type=0,
                    heading=utils.getString(30120),
                    shares="files",
                    defaultt=backupSet["root"],
                )
                ...
                if addFolder.startswith(rootPath):
                    if not any(addFolder == aDir["path"] for aDir in backupSet["dirs"]):
                        ...
                        if optionSelected == 0 and addFolder != backupSet["root"]:
                            backupSet["dirs"].append(
                                {"path": addFolder, "type": "exclude"}
                            )
                        elif optionSelected == 1:
                            # can add root as inclusion
                            backupSet["dirs"].append(
                                {"path": addFolder, "type": "include", "recurse": True}
                            )
                    else:
                        # this path is already part of another include/exclude rule
                        self.dialog.ok(
                            utils.getString(30117), utils.getString(30137), addFolder
                        )
                else:
                    # folder must be under root folder
                    self.dialog.ok(
                        utils.getString(30117), utils.getString(30136), rootPath
                    )
            elif optionSelected == 2:
                self.dialog.ok(
                    utils.getString(30121), utils.getString(30130), backupSet["root"]
                )
            elif optionSelected > 2:
                cOptions = ["Delete"]
                ...
                if contextOption == 0:
                    if self.dialog.yesno(
                        heading=utils.getString(30123), message=utils.getString(30128)
                    ):
                        # remove folder
                        del backupSet["dirs"][optionSelected - 3]
                elif (
                    contextOption == 1
                    and backupSet["dirs"][optionSelected - 3]["type"] == "include"
                ):
                    # toggle if this folder should be recursive
                    backupSet["dirs"][optionSelected - 3]["recurse"] = not backupSet[
                        "dirs"
                    ][optionSelected - 3]["recurse"]
    ...
    def showMainScreen(self):
        ...
        while exitCondition != -1:
            ...
            listItem.setArt(
                {
                    "icon": os.path.join(
                        utils.addon_dir(), "resources", "images", "plus-icon.png"
                    )
                }
            )
            options = [listItem]
            ...
            for index in range(0, len(customPaths.getSets())):
                ...
                listItem = xbmcgui.ListItem(
                    aSet["name"], utils.getString(30121) + ": " + aSet["set"]["root"]
                )
                listItem.setArt(
                    {
                        "icon": os.path.join(
                            utils.addon_dir(), "resources", "images", "folder-icon.png"
                        )
                    }
                )
            ...
            # show the gui
            exitCondition = self.dialog.select(
                utils.getString(30125), options, useDetails=True
            )
            ...
            if exitCondition >= 0:
                if exitCondition == 0:
                    ...
                    if customPaths.validateSetName(newSet["name"]):
                        ...
                        self.dialog.ok(
                            utils.getString(30117),
                            utils.getString(30138),
                            newSet["name"],
                        )
                else:
                    # bring up a context menu
                    menuOption = self.dialog.contextmenu(
                        [utils.getString(30122), utils.getString(30123)]
                    )
                    ...
                    if menuOption == 0:
                        ...
                        if self.dialog.yesno(
                            heading=utils.getString(30127),
                            message=utils.getString(30128),
                        ):
    ...
    def copySimpleConfig(self):
        ...
        shouldContinue = self.dialog.yesno(
            heading=utils.getString(30139),
            message=utils.getString(30140) + "\n" + utils.getString(30141),
        )
        ...
        if shouldContinue:
            ...
            dest = xbmcvfs.translatePath(
                os.path.join(utils.data_dir(), "custom_paths.json")
            )
